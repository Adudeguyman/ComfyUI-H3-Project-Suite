...
**Anchor audio (unwired)** — experimental, for a voice that drifts down a chain. Each clip's sound is generated from the previous clip's generated sound, so the model keeps continuing its own approximation and a known character slowly stops sounding like themselves. Wire a clean sample of the voice here, from clip 1 or a real recording, and it is given to the model as a reference on every clip, including the first, so identity is pulled back to the same source each time. The carried-over tail still continues the sound; this says what it should keep sounding like. Needs the audio VAE wired. The first ten seconds are used, and the prompt can't refer to it by tag. Use **Measure drift** in the panel, which now reports sound as well as picture, to see whether it helped.
